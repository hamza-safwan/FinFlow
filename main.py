...
def main():
    ...
    accounts.append((randint(5000, 5000000)))
    accounts.append(OverdraftAccount(randint(5000, 5000000)))
    for i in range(20):
        account = accounts[randint(0, 4)]
        choice = randint(0, 5)
        if choice == 0: account.deposit(randint(100, 5000000))
            ...
            if account_type == 'CURRENT' or account_type == 'OD':
                account.debit_bank_charges()
            elif account_type == 'SAVING' or account_type == 'S_SAVING':
